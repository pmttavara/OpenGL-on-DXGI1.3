...
int main()
{
    ...
    *(void**)&glFramebufferRenderbuffer = (void*)wglGetProcAddress("glFramebufferRenderbuffer");
    ...
    for (;;)
    {
        ...
        IDXGIDebug1 * pDebug;
        hr = DXGIGetDebugInterface1(0, IID_IDXGIDebug, (void **)&pDebug);
        Assert(SUCCEEDED(hr));
        hr = IDXGIDebug_ReportLiveObjects(pDebug, DXGI_DEBUG_ALL, DXGI_DEBUG_RLO_SUMMARY);
        Assert(SUCCEEDED(hr));
        hr = IDXGIDebug_Release(pDebug);
        Assert(SUCCEEDED(hr));
    }
